  ...
  if (!profile) {
    return (
      <div className="pt-40 pb-20 px-6 min-h-screen text-center flex flex-col items-center justify-center">
        ...
        <Link href="/register" className="px-8 py-3 bg-[#D4A435] text-black text-xs uppercase tracking-[2px] font-medium hover:bg-[#b0872a] transition-colors shadow-[0_0_15px_rgba(212,164,53,0.3)]">
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="mb-10 flex flex-col space-y-2">
          <h1 className="text-3xl md:text-4xl font-light tracking-widest uppercase text-white">
            Artist <span className="text-[#D4A435]">Profile</span>
          </h1>
          <div className="w-16 h-[1px] bg-[#D4A435] opacity-50"></div>
        </div>

        <div className="overflow-hidden bg-black border border-[#D4A435]/30 shadow-2xl relative">
          
          <div className="md:flex">
            {/* Left Sidebar Profile Card */}
            <div className="md:w-1/3 bg-zinc-950/50 p-8 flex flex-col items-center text-center border-b md:border-b-0 md:border-r border-[#D4A435]/30 relative">
              ...
              <div className="absolute top-4 left-4">
                <div className={`uppercase inline-block border text-[9px] tracking-[2px] px-2 py-0.5 ${
                  ...
                  'border-[#D4A435] text-[#D4A435] bg-[#D4A435]/10'
                }`}>
              ...
              <div className="mt-8 mb-6">
                {user.avatar ? (
                  <img src={user.avatar} alt="Profile" className="w-36 h-36 rounded-full border border-[#D4A435] object-cover p-1 shadow-[0_0_20px_rgba(212,164,53,0.15)]" />
                ) : (
                  <div className="w-36 h-36 rounded-full bg-black border border-[#D4A435] flex items-center justify-center text-4xl font-light text-[#D4A435] tracking-widest uppercase p-1 shadow-[0_0_20px_rgba(212,164,53,0.15)]">
                    {profile.name[0]}
                  </div>
                )}
              </div>
              ...
              <p className="text-[#D4A435] text-xs tracking-[2px] uppercase mt-2">{profile.category || "General Artist"}</p>
              
              <div className="mt-auto pt-10 w-full flex flex-col gap-3">
                <Link href="/register" className="text-[10px] uppercase tracking-[3px] border border-[#D4A435]/50 text-[#D4A435] py-3 hover:bg-[#D4A435] hover:text-black transition-all w-full inline-block text-center">
                  Edit Profile
                </Link>
              </div>
            </div>
            ...
            <div className="md:w-2/3 p-8 md:p-12 space-y-10 bg-black">
              ...
              {/* Bio Section */}
              <div>
                <h3 className="text-[10px] uppercase tracking-[3px] text-[#D4A435] mb-4 flex items-center gap-4">
                  Professional Bio
                  <div className="h-[1px] bg-[#D4A435]/20 flex-1"></div>
                </h3>
              ...
              {/* Grid Demographics */}
              <div>
                <h3 className="text-[10px] uppercase tracking-[3px] text-[#D4A435] mb-6 flex items-center gap-4">
                  Personal Details
                  <div className="h-[1px] bg-[#D4A435]/20 flex-1"></div>
                </h3>
              ...
              {(profile.social_link || profile.portfolio_link) && (
                <div>
                  ...
                  <div className="flex flex-wrap gap-4">
                    {profile.social_link && (
                      <a href={profile.social_link.startsWith('http') ? profile.social_link : `https://${profile.social_link}`} target="_blank" rel="noopener noreferrer" className="px-6 py-2.5 bg-zinc-900 border border-zinc-800 hover:border-[#D4A435]/50 text-white text-[10px] uppercase tracking-[2px] transition-colors flex items-center gap-2">
                        Social Media
                        <span className="text-[#D4A435]">↗</span>
                      </a>
                    )}
                    {profile.portfolio_link && (
                      <a href={profile.portfolio_link.startsWith('http') ? profile.portfolio_link : `https://${profile.portfolio_link}`} target="_blank" rel="noopener noreferrer" className="px-6 py-2.5 bg-zinc-900 border border-zinc-800 hover:border-[#D4A435]/50 text-white text-[10px] uppercase tracking-[2px] transition-colors flex items-center gap-2">
                        Portfolio
                        <span className="text-[#D4A435]">↗</span>
                      </a>
                    )}
