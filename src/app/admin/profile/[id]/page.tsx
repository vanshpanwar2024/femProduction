  ...
  if (!profile) {
    return (
      <div className="pt-32 pb-20 px-6 min-h-screen relative z-10 max-w-4xl mx-auto text-center space-y-6">
        ...
        <Link href="/admin" className="inline-block text-[#D4A435] uppercase tracking-widest text-sm hover:underline">
          Return to Dashboard
        </Link>
      </div>
    );
  ...
  return (
    <div className="pt-32 pb-20 px-6 min-h-screen relative z-10 max-w-4xl mx-auto flex flex-col items-center">
      <div className="w-full mb-8 flex items-center justify-between">
        <Link href="/admin" className="text-zinc-400 hover:text-[#D4A435] text-xs uppercase tracking-[3px] transition-colors flex items-center gap-2">
        ...
        <div className="p-8 md:p-12 space-y-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-8">
            <div className="space-y-2 text-sm">
              <p className="text-[#D4A435] text-[10px] uppercase tracking-[3px]">Age</p>
              <p className="text-white text-lg font-light">{profile.age || "N/A"}</p>
            </div>
            <div className="space-y-2 text-sm">
              <p className="text-[#D4A435] text-[10px] uppercase tracking-[3px]">Date of Birth</p>
              <p className="text-white text-lg font-light">{profile.dob ? new Date(profile.dob).toLocaleDateString() : "N/A"}</p>
            </div>
            <div className="space-y-2 text-sm">
              <p className="text-[#D4A435] text-[10px] uppercase tracking-[3px]">Gender</p>
              <p className="text-white text-lg font-light capitalize">{profile.gender || "N/A"}</p>
            </div>
            <div className="space-y-2 text-sm">
              <p className="text-[#D4A435] text-[10px] uppercase tracking-[3px]">Category</p>
              <p className="text-white text-lg font-light capitalize">{profile.category || "N/A"}</p>
            </div>
          </div>

          <div className="space-y-2">
            <p className="text-[#D4A435] text-[10px] uppercase tracking-[3px]">Artist Bio</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-4">
            {profile.social_link && (
              <div className="space-y-2">
                <p className="text-[#D4A435] text-[10px] uppercase tracking-[3px]">Social Profile</p>
                <a href={profile.social_link} target="_blank" rel="noopener noreferrer" className="text-white hover:text-[#D4A435] text-sm break-all border-b border-zinc-700 pb-1 inline-block transition-colors">
                  {profile.social_link}
                </a>
              </div>
            )}
            ...
            {profile.portfolio_link && (
              <div className="space-y-2">
                <p className="text-[#D4A435] text-[10px] uppercase tracking-[3px]">Portfolio / Video Link</p>
                <a href={profile.portfolio_link} target="_blank" rel="noopener noreferrer" className="text-white hover:text-[#D4A435] text-sm break-all border-b border-zinc-700 pb-1 inline-block transition-colors">
                  {profile.portfolio_link}
                </a>
              </div>
            )}
